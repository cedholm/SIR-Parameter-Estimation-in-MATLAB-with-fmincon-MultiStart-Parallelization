...
%% Record the outputs from MultiStart/fmincon in one Excel file

% If you are doing multiple runs, change this to save different files
RunCounter=1;

header={'beta','fvals','EF'};

OutputExcel=[SIRParameters,fvalues',ExitFlags'];

filename =['SIR_Outputs_RunCounter_',num2str(RunCounter), '.xlsx'];
writecell(header,filename,'Sheet',1,'Range','A1');
writematrix(OutputExcel,filename,'Sheet',1,'Range','A2');
...
filenamefig =['SIR-vary-beta_RunCounter_',num2str(RunCounter)];

saveas(gcf, filenamefig,'fig')
saveas(gcf, filenamefig,'epsc')     
...
value=norm(diff,2)/norm(ConfCase,2);
